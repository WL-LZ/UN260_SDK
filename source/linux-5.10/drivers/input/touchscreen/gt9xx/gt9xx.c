			 ...
			 u8 *key_value)
{
	...
	dev_info(&ts->client->dev, "num=%d x=%d y=%d size=%d\n", 
			touch_num,
			point_data[GTP_ADDR_LENGTH + 2] + (point_data[GTP_ADDR_LENGTH + 3] << 8),
			point_data[GTP_ADDR_LENGTH + 4] + (point_data[GTP_ADDR_LENGTH + 5] << 8),
			point_data[GTP_ADDR_LENGTH + 6] + (point_data[GTP_ADDR_LENGTH + 7] << 8));
	if (touch_num > 1) {
