 ...
 *  STATIC PROTOTYPES
 **********************/
static lv_img_draw_status_cb_t draw_status_cb;
static bool (*draw_retry_cb)(lv_obj_t *obj);
void lv_img_set_draw_status_cb(lv_img_draw_status_cb_t cb) { draw_status_cb=cb; }
void lv_img_set_draw_retry_cb(bool (*cb)(lv_obj_t *obj)) { draw_retry_cb=cb; }
...
void lv_img_set_src(lv_obj_t * obj, const void * src)
{
    ...
    if(header_res != LV_RES_OK && src_type != LV_IMG_SRC_SYMBOL) {
        /* A first-open failure must still have drawable geometry and a retry
         * owner. Do not leave an invisible 0x0 object or divide by zero later. */
        header.w = img->w > 0 ? img->w : 32;
        header.h = img->h > 0 ? img->h : 32;
    }
    ...
    lv_obj_invalidate(obj);
    if(header_res != LV_RES_OK && src_type != LV_IMG_SRC_SYMBOL && draw_status_cb) draw_status_cb(obj,false);
}
...
static void draw_img(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_COVER_CHECK) {
        ...
        if(code == LV_EVENT_DRAW_MAIN) {
            if(draw_retry_cb && !draw_retry_cb(obj)) {
                lv_draw_img(lv_event_get_draw_ctx(e),NULL,&obj->coords,NULL);
                if(draw_status_cb) draw_status_cb(obj,false);
                return;
            }
            if(img->h == 0 || img->w == 0) {
                lv_draw_img(lv_event_get_draw_ctx(e), NULL, &obj->coords, NULL);
            ...
            if(img->src_type == LV_IMG_SRC_FILE || img->src_type == LV_IMG_SRC_VARIABLE) {
                ...
                for(; coords_tmp.y1 < img_max_area.y2; coords_tmp.y1 += img_size_final.y, coords_tmp.y2 += img_size_final.y) {
                    ...
                    for(; coords_tmp.x1 < img_max_area.x2; coords_tmp.x1 += img_size_final.x, coords_tmp.x2 += img_size_final.x) {
                        if(lv_draw_img_checked(draw_ctx, &img_dsc, &coords_tmp, img->src) != LV_RES_OK) draw_ok=false;
                    }
                }
                draw_ctx->clip_area = clip_area_ori;
                if(draw_status_cb) draw_status_cb(obj,draw_ok);
            }
