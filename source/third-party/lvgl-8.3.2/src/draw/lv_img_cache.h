...
typedef struct {
    ...
    bool retained;
    uint16_t pins; /**< Opening/drawing entries cannot be reclaimed. */
...
/* Optional decoder-specific allocated-byte accounting. UI-thread only. */
typedef uint32_t (*lv_img_cache_memory_cb_t)(const lv_img_decoder_dsc_t *dsc);
void lv_img_cache_set_memory_cb(lv_img_cache_memory_cb_t cb);
...
void lv_img_cache_pin(_lv_img_cache_entry_t *entry);
