...
static const char *b0_text(uint8_t status)
{
    switch (status) {
...
static bool b0_terminal(uint8_t status)
{
...
static const upgrade_page_runtime_config_t g_upgrade_config = {
    .command = 0xB0, .timeout_ms = 40000,
    .status_text = b0_text, .is_terminal = b0_terminal,
    .page_id = UI_PAGE_IMAGE_UPGRADE,
    .owner = UPGRADE_SESSION_IMAGE,
};

void ui_page_15_image_upgrade_on_reply(uint8_t command, uint8_t status)
{
    if (command == 0xB0) upgrade_page_runtime_handle_reply(&g_upgrade_runtime, status);
}
...
void ui_page_15_image_upgrade_create(lv_obj_t *parent)
{
    upgrade_page_runtime_create(&g_upgrade_runtime, parent, &g_upgrade_config,
        "Image-board update", device_info_is_valid() ? device_info_image_app() : NULL);
}
