...
#include "boot_d_motion.h"
#include "un260/lv_core/lv_page_manager.h"
#include "un260/lv_core/page_08_boot.h"

/* Finish all three staggered dot cycles before the symmetric text exit. */
#define INTRO_REVEAL_MS BOOT_WELCOME_HOLD_END
#define INTRO_FADE_MS 650U
...
static const char background_src[] =
    "L:/usr/local/share/lvgl_data/boot_theme_d/background.png";
static const char emblem_src[] =
    "L:/usr/local/share/lvgl_data/boot_theme_d/emblem.png";
...
static struct {
    ...
    lv_timer_t *timer;
    uint32_t start_tick;
    bool background_ready, icon_ready, selftest_covered, selftest_prepared, first_drawn;
...
static void root_deleted(lv_event_t *event)
{
    ...
    /* The self-test owns the same background after an overlay handoff.
     * Keep that cache alive; its page destructor releases it after use. */
...
static void apply_elapsed(uint32_t elapsed)
{
    ...
    /* Fixed glyph geometry: opacity only, no subpixel text movement. */

    if (g_intro.background_ready)
        image_opacity(g_intro.background, LV_OPA_COVER);
    else background_opacity(g_intro.root, LV_OPA_COVER);
    if (g_intro.icon_ready) image_opacity(g_intro.icon, LV_OPA_TRANSP);
    text_opacity(g_intro.brand, LV_OPA_TRANSP);
    text_opacity(g_intro.welcome, opacity(welcome * foreground));

    for (unsigned i = 0; i < 3; ++i) {
        float visible = ease(progress(elapsed,2300U,800U));
        float jump = boot_d_dot(elapsed,i);
        lv_coord_t y=BOOT_DOT_Y-(lv_coord_t)(BOOT_WELCOME_MOTION_SCALE*5.0f*jump+.5f);
...
static void timer_cb(lv_timer_t *timer)
{
    ...
    if (g_intro.selftest_covered && !g_intro.selftest_prepared && g_intro.first_drawn && quiet) {
        if (ui_page_08_curr_prepare_step()) {
            g_intro.selftest_prepared = true;
        }
    }
    if (g_intro.managed && g_intro.startup_ready && !g_intro.revealing &&
        (!g_intro.selftest_covered || g_intro.selftest_prepared) &&
                       ...
                       const lv_font_t *font, int spacing)
{
    ...
    lv_obj_set_style_text_color(obj, lv_color_hex(0x526D82), 0);
...
void ui_page_00_boot_anim_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_bg_color(g_intro.root, lv_color_hex(0xF2F5F7), 0);
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        lv_obj_set_style_bg_color(g_intro.dots[i], lv_color_hex(0x7C879E), 0);
        background_opacity(g_intro.dots[i], LV_OPA_TRANSP);
...
#endif /* UI_BOOT_ANIM_THEME == UI_BOOT_ANIM_THEME_D */
