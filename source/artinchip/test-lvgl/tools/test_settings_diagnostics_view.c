...
int protocol_send(uint8_t cmd,const uint8_t *sub,uint16_t len){settings_detail_send_command(cmd,sub,len);return len+5;}
...
static void motor_test(void){
 ui_page_17_motor_test_create(lv_scr_act());snapshot("motors-ready");click("Forward");assert(sent_cmd==0x52&&holds&2);ui_page_17_motor_test_on_reply(0x52,1);assert(label_find(lv_scr_act(),"Start accepted"));snapshot("motors-accepted");ui_page_17_motor_test_destroy();ui_page_17_motor_test_on_reply(0x52,1);ui_page_17_motor_test_on_reply(0x53,1);ui_page_17_motor_test_on_reply(0x54,1);assert(!(holds&14));puts("PASS motor ACK, leave stop commands and late ACK owner release");
}
...
static void debug_test(void){
 ui_page_10_debug_create();snapshot("debug-communication");for(unsigned i=0;i<205;i++)debug_append_rx_log("FD DF 06 38 01 26");
...
static void failure_retry_test(void){
 ...
 ui_page_17_motor_test_create(lv_scr_act());click("Retry");ui_page_17_motor_test_destroy();
