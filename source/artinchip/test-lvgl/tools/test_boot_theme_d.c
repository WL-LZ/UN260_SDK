...
static lv_res_t info(lv_img_decoder_t *d,const void *s,lv_img_header_t *h){
 ...
 bool full=strstr(s,"background.png")!=NULL;if(!full&&!strstr(s,"emblem.png"))return LV_RES_INV;
 memset(h,0,sizeof(*h));h->cf=LV_IMG_CF_TRUE_COLOR_ALPHA;h->w=full?1280:96;h->h=full?400:96;return LV_RES_OK;
}
static lv_res_t open_image(lv_img_decoder_t *d,lv_img_decoder_dsc_t *s){if(info(d,s->src,&s->header)!=LV_RES_OK)return LV_RES_INV;s->img_data=s->header.w==1280?bg:icon;opens++;return LV_RES_OK;}
...
int main(void){
 ...
 for(unsigned i=0;i<5;i++){
  ...
  pixels=0;unsigned decodes=opens;for(unsigned j=0;j<30;j++)frame(14);assert(pixels==0&&opens==decodes);
 ...
 snapshot.stage=BOOT_STAGE_DONE;frame(20);assert(!ui_page_08_curr_visual_is_quiet());for(unsigned i=0;i<70;i++)frame(14);assert(!strcmp(lv_label_get_text(view.title),"Ready."));dump("ready");
 for(unsigned i=0;i<70;i++)frame(14);assert(ui_page_08_curr_visual_is_quiet());
 assert(lv_obj_get_style_bg_opa(view.track,0)==0);assert(lv_obj_get_style_text_opa(view.section,0)==0);dump("ready-clean");
 ui_page_08_curr_start_handoff();current=UI_PAGE_MAIN;ui_page_08_curr_destroy();frame(200);
 assert(bridge.root&&lv_obj_get_style_img_opa(bridge.image,0)==255);
 frame(200);assert(lv_obj_get_style_text_opa(bridge.text,0)==0);dump("handoff-text-retired");
 for(unsigned i=0;i<50;i++)frame(14);assert(!bridge.root&&timers()==baseline);
