...
with tempfile.TemporaryDirectory(prefix='un260-standby-view-') as temp:
    ...
    sources=[root/'tools/test_standby_view.c',root/'un260/font/scaled_font.c',*[root/'un260/font'/f'{f}.c' for f in fonts],*lvgl.joinpath('src').rglob('*.c')]
    common=['gcc','-std=gnu11','-O1','-g','-fsanitize=undefined','-fno-sanitize-recover=all',f'-I{work}',f'-I{root}',f'-I{lvgl}',f'-DLV_CONF_PATH={conf}']
