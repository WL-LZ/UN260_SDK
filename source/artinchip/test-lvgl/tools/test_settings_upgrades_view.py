#!/usr/bin/env python3
"""Actual production upgrade pages and LVGL renderer; updater and UART are mocked boundaries."""
...
pages=['un260/lv_core/'+p for p in ['page_14_main_upgrade.c','page_15_image_upgrade.c','page_16_ui_upgrade.c','upgrade_page_runtime.c']]
parts=[*pages,
...
with tempfile.TemporaryDirectory(prefix='un260-settings-view-') as temp:
    ...
    (work/'actual_upgrade_pointer.h').write_text(function((root/'lv_port_indev.c').read_text(),'lv_port_indev_set_drag_obj'))
    ...
    sources=[*compiled_asset_sources(),root/'tools/test_settings_upgrades_view.c',*[root/p for p in pages],root/'un260/lv_components/lv_settings.c',
      root/'un260/app_service/upgrade_session.c',
